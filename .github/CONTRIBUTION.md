# Contributing to MC Group IO

Thank you for your interest in contributing to MC Group IO!
This document explains how to set up your environment and contribute changes to this monorepo.
...
2. Clone your fork:
   ```bash
   git clone https://github.com/<your-username>/MC_Group_IO.git
   cd MC_Group_IO
   ```
3. Install dependencies:
   ...
   npm install
   ```
4. Set up environment variables as described in the [README.md](../README.md)

## Development Workflow

1. Create a new branch for your feature or bugfix (see [Branch Naming Convention](#branch-naming-convention))
2. Start the dev environment if needed:
   ```bash
   npm run dev
   ```
   This runs the monorepo in development (apps/services as configured).
3. Make your changes in the relevant app under `apps/` or service under `services/`.
4. Run checks to ensure code quality from the repo root:
   ```bash
   npm run lint
...
7. Open a Pull Request (the repository uses a PR template)
...
For testing a specific application or service, run the test script from that workspace directory, e.g.:

```bash
cd apps/reuse-x && npm run test
```
